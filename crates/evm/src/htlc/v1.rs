...
/// Convert an EVMSwap to an Initiate struct
impl From<EVMSwap> for Initiate {
    fn from(swap: EVMSwap) -> Self {
        Self {
...
impl HTLCInterface for GardenHTLCContract {
    ...
    fn get_order_id(
        ...
        swap: &EVMSwap,
        _asset: &Address,
    ) -> Result<alloy::primitives::FixedBytes<32>, HTLCError> {
        let components = (chain_id, swap.secret_hash, swap.initiator);
